...
uint32_t dbg_generate_random(void) {
    return rand();
}
...
void dbg_generate_random_payload(uint32_t pkt_cnt, uint8_t * buffer_expected, uint8_t size) {
    ...
    srand(pkt_cnt); // Use pkt_cnt as seed for pseudo-randomness

    for (k = 8; k < (int)size; k++) {
        buffer_expected[k] = (uint8_t)dbg_generate_random(); // Generate a random byte
    }
}
